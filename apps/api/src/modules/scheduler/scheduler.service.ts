...
export class SchedulerService {
  ...
  private resolveWeights(strategy: SchedulingStrategy, custom?: SchedulingWeights): SchedulingWeights {
    ...
    switch (strategy) {
      case SchedulingStrategy.CHEAPEST:
        return { costWeight: 0.8, performanceWeight: 0.1, reliabilityWeight: 0.1 };
      case SchedulingStrategy.BEST_PERFORMANCE:
        return { costWeight: 0.1, performanceWeight: 0.8, reliabilityWeight: 0.1 };
      case SchedulingStrategy.HIGHEST_RELIABILITY:
        return { costWeight: 0.1, performanceWeight: 0.2, reliabilityWeight: 0.7 };
  ...
  async evaluateCandidates(dto: EvaluateScheduleDto): Promise<SchedulingDecision> {
    ...
    const nodesToScore = eligibleNodes.length > 0 ? eligibleNodes : allCandidateNodes;
    ...
    const scoredCandidates: CandidateNodeScore[] = nodesToScore.map((n) => {
      ...
      // Reliability score: 99.8% normalized [0, 1]
      const reliabilityScoreNormalized = 0.998;
